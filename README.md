...
## Example for SFTP Server with Custom Identity Provider (Cognito + Lambda)

This example demonstrates a complete custom identity provider solution using AWS Cognito, Lambda, and DynamoDB for user authentication and authorization.

**Example location**: `examples/sftp-idp-cognito-lambda`

### Key Features

- **Custom Identity Provider**: Lambda-based authentication with Cognito integration
- **DynamoDB User Management**: Flexible user configuration with home directories and IAM roles
- **Modular Architecture**: Uses both transfer-server and transfer-custom-idp-solution modules
- **Security Compliance**: Latest security policies and KMS encryption
- **IP Allow Lists**: Per-user IP restrictions for enhanced security
...
module "custom_idp" {
  ...
  users_table_name              = ""  # Creates new table
  identity_providers_table_name = ""  # Creates new table
  create_vpc                    = false
  use_vpc                       = false
  provision_api                 = false
...
Users are configured via DynamoDB with support for:
- Custom home directories (logical paths)
- IAM role assignments
- IP allow lists per user
- Integration with Cognito for authentication

## Transfer Custom Identity Provider Solution Module

The `transfer-custom-idp-solution` module provides a complete custom identity provider infrastructure:

### Core Components

- **Lambda Function**: Handles authentication requests from Transfer Family
- **DynamoDB Tables**: Stores user configurations and identity provider settings
- **CodeBuild Integration**: Automatically builds Lambda artifacts from AWS Transfer Family Toolkit
- **Optional VPC Support**: Can create new VPC or use existing infrastructure
- **Optional API Gateway**: For REST API-based identity provider

### Security Features

- **KMS Encryption**: All DynamoDB tables and S3 buckets encrypted with KMS
- **VPC Flow Logging**: Optional VPC flow logging for network monitoring
- **Default Security Group**: Restrictive default security group configuration
- **Secrets Manager Integration**: Secure credential storage support
- **X-Ray Tracing**: Optional distributed tracing support

### Usage Patterns

```hcl
# Basic Lambda-only setup
module "custom_idp" {
  source = "aws-ia/transfer-family/aws//modules/transfer-custom-idp-solution"
