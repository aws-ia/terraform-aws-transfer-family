...
Create a `users.csv` file (available in `examples/sftp-malware-protection-guardduty`) with the following format:
...
**Note**: The `users.csv` file is required. All other variables have defaults in `variables.tf` but can be overridden in `terraform.tfvars` to customize the malware protection behavior, file organization, resource naming, and security settings.
