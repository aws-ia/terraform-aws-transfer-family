...
| [aws_s3control_access_grant.group_access](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/s3control_access_grant) | resource |
| [aws_s3control_access_grants_location.bucket](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/s3control_access_grants_location) | resource |
...
| [aws_ssoadmin_application_assignment.group_assignments](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/ssoadmin_application_assignment) | resource |
...
|------|-------------|------|---------|:--------:|
| <a name="input_access_grant_permissions"></a> [access\_grant\_permissions](#input\_access\_grant\_permissions) | Map of group names to their S3 permissions | `map(string)` | <pre>{<br/>  "admins": "READWRITE",<br/>  "analysts": "READ"<br/>}</pre> | no |
...
| <a name="input_group_memberships"></a> [group\_memberships](#input\_group\_memberships) | Map of group memberships (group\_key -> list of user\_keys) | `map(list(string))` | <pre>{<br/>  "admins": [<br/>    "admin"<br/>  ],<br/>  "analysts": [<br/>    "analyst"<br/>  ]<br/>}</pre> | no |
| <a name="input_groups"></a> [groups](#input\_groups) | Map of groups to create | <pre>map(object({<br/>    display_name = string<br/>    description  = string<br/>  }))</pre> | <pre>{<br/>  "admins": {<br/>    "description": "Full access to all files and administrative functions",<br/>    "display_name": "Administrators"<br/>  },<br/>  "analysts": {<br/>    "description": "Read access to shared files and write access to analysis folders",<br/>    "display_name": "Analysts"<br/>  }<br/>}</pre> | no |
...
| <a name="input_users"></a> [users](#input\_users) | Map of users to create with their email addresses | <pre>map(object({<br/>    display_name = string<br/>    given_name   = string<br/>    family_name  = string<br/>    email        = string<br/>  }))</pre> | <pre>{<br/>  "admin": {<br/>    "display_name": "Admin User",<br/>    "email": "admin@example.com",<br/>    "family_name": "User",<br/>    "given_name": "Admin"<br/>  },<br/>  "analyst": {<br/>    "display_name": "Analyst User",<br/>    "email": "analyst@example.com",<br/>    "family_name": "User",<br/>    "given_name": "Analyst"<br/>  }<br/>}</pre> | no |
