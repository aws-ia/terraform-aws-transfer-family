...
module "custom_idp" {
  ...
  use_vpc         = false
  provision_api   = true  # Use API Gateway
  enable_tracing  = true
...
resource "aws_dynamodb_table_item" "cognito_idp_config" {
  ...
  item = jsonencode({
    ServerId = {
      S = "placeholder-server-id"  # Will be updated after server creation
    }
    config = {
      M = {
        cognito_client_id = {
...
# Update the server ID after creation
resource "aws_dynamodb_table_item" "cognito_idp_config_update" {
  table_name = module.custom_idp.identity_providers_table_name
  hash_key   = module.custom_idp.identity_providers_table_hash_key
  ...
  depends_on = [aws_transfer_server.sftp, aws_dynamodb_table_item.cognito_idp_config]
}
