...
| <a name="input_users"></a> [users](#input\_users) | List of SFTP users. Each user can have either a single public\_key or multiple public\_keys, but not both. | <pre>list(object({<br/>    username    = string<br/>    home_dir    = string<br/>    public_key  = optional(string)<br/>    public_keys = optional(list(string))<br/>    role_arn    = optional(string)<br/>  }))</pre> | `[]` | no |
