...
class CognitoIdpModuleError(Exception):
...
def handle_auth(
    ...
    # This module only supports password authentication
    if event.get("password", "").strip() == "":
        raise CognitoIdpModuleError("This provider requires password authentication.")    
    ...
    try:
        ...
        if response.get("AuthenticationResult", None) is None:
            if "ChallengeName" in response:
                if cognito_mfa is None:
                    ...
                    raise CognitoIdpModuleError(f"Unsupported challenge type {response['ChallengeName']}. SOFTWARE_TOKEN_MFA is the only supported challenge type.")                
            else:
        ...
        logger.error(f"Authentication failed for user {parsed_username}: Invalid credentials")
        raise CognitoIdpModuleError("Invalid username or password")
    except client.exceptions.UserNotFoundException:
        logger.error(f"User {parsed_username} not found in Cognito")
        raise CognitoIdpModuleError("User not found")
    except Exception as e:
        logger.error(f"Cognito authentication error: {str(e)}")
        raise CognitoIdpModuleError(f"Authentication failed: {str(e)}")
